    ...
    for x, y, x1, y1 in faces:
        ...
        if 0 not in measures:
            ...
            if np.mean(measures) >= 0.7:
                ...
                #print(tc)
                cv2.putText(img=img, text=text, org=point, fontFace=font, fontScale=0.9, color=(0, 0, 255),thickness=2, lineType=cv2.LINE_AA)
            else:
                font = cv2.FONT_HERSHEY_SIMPLEX
                if tc==1:
                    stop=time.time()
                    ...
                    tc=0
                #print(tc)    
                cv2.putText(img=img, text=text, org=point, fontFace=font, fontScale=0.9,color=(0, 255, 0), thickness=2, lineType=cv2.LINE_AA)
